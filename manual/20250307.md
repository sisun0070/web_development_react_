...
그리고 다양한 ag grid 관련 css 중에 meterial 디자인 스타일 적용을 하기 위해
...
칼럼 정의 객체의 배열인 columnDefs라는 상태를 정의할겁니다. ag-grid는 여기에 이용할 수 있는 ColDef 타입을 제공합니다. 컬럼 객체에서는 필수 field 프롭을 이용하여 데이터 접근자를 정의해야 합니다. field 값은 컬럼이 표시해야 하는 REST API 응답 데이터의 속성 이름에 해당합니다.

```tsx
import { ColDef } frp, 'ag-grid-community';
